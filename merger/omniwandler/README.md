...
Der **OmniWandler** (ehemals all-ein-wandler) ist das Gegenstück zum `repoLens` (vormals `wc-merger`). Während `repoLens` für Code-Repositories optimiert ist, kümmert sich der `OmniWandler` um **Inhalts-Ordner**:
